...
tests_add_filter(
	...
	static function (): void {
		// Load Contact Form 7 (a sibling plugin in the test env) FIRST so the CF7
		...
		// here. Guarded so the suite still boots if CF7 is absent (its tests then
		// skip), matching how the abilities themselves degrade.
		$cf7 = dirname(TESTS_REPO_ROOT_DIR) . '/contact-form-7/wp-contact-form-7.php';
		if (is_readable($cf7)) {
			require_once $cf7;
		}
