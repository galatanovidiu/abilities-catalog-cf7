...
/**
 * The integration registers a `contact-form-7` domain tool and contributes a knowledge bundle to
...
final class IntegrationTest extends TestCase {
	...
	/**
	 * contributeDomain registers a described `contact-form-7` tool with its abilities.
	 *
	 * @return void
	 */
	public function test_contribute_domain_registers_a_described_cf7_tool(): void {
		...
		$this->assertArrayHasKey( 'contact-form-7', $domains );
		$this->assertIsString( $domains['contact-form-7']['description'] );
		$this->assertNotEmpty( $domains['contact-form-7']['description'] );
		$this->assertNotEmpty( $domains['contact-form-7']['abilities'] );
	}

	/**
	 * Every ability the `contact-form-7` tool lists is a registered ability (no name drift).
	 *
	 * @return void
	 */
	public function test_every_listed_ability_is_registered(): void {
		...
		foreach ( $domains['contact-form-7']['abilities'] as $name ) {
			$this->assertTrue(
				wp_has_ability( $name ),
				sprintf( 'The contact-form-7 tool lists "%s", which is not a registered ability.', $name )
			);
		}
	}

	/**
	 * contributeKnowledge appends a scanned `contact-form-7` bundle carrying the recipe.
	 *
	 * @return void
	 */
	public function test_contribute_knowledge_adds_the_bundle(): void {
		...
		$this->assertSame( 'contact-form-7', $bundle->slug() );

		$concept = $bundle->concept( 'set-up-contact-form' );
		$this->assertNotNull( $concept, 'The contact-form-7 bundle must carry the set-up-contact-form concept.' );
		$this->assertSame( 'contact-form-7/set-up-contact-form', $concept->uri() );
