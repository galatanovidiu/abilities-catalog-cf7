...
abilities and a **set-up-contact-form** skill (a recipe that chains finding or
creating a form into placing its shortcode on a page), through the catalog's public
filters. No core files of Abilities Catalog are modified.
...
registered-and-denying), and the forms domain tool and skill do not appear.
...
3. Optional: install Abilities Catalog and enable its MCP server to expose the
   forms domain tool and the set-up-contact-form skill.

== Changelog ==

= 0.1.0 =
* Initial release: six CF7 form abilities, plus optional Abilities Catalog MCP
  integration (forms domain tool and set-up-contact-form skill).
