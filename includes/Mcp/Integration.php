 ...
 * it registers a `contact-form-7` domain tool — its description and the `og-cf7/*` abilities it
 ...
 * is inactive the `og-cf7/*` abilities do not register and no empty `contact-form-7` tool or
...
final class Integration {
	...
	/**
	 * The exact `og-cf7/*` ability names the `contact-form-7` domain tool owns, in tool order.
	 *
	 * @var list<string>
	 */
	...
	/**
	 * Registers the `contact-form-7` domain tool — its description and the abilities it owns.
	 *
	 * One call defines the whole tool: the server builds a `contact-form-7` tool, routes the
	 * `og-cf7/*` abilities to it, and uses the description as the tool's routing blurb.
	 ...
	 * @return array<string, array{description: string, abilities: list<string>}> The map including the `contact-form-7` tool.
	 */
	 ...
	 * single `set-up-contact-form` recipe that chains the `contact-form-7` domain into `content`
	 ...
	 * under the `contact-form-7` slug. A failed scan ({@see KnowledgeBundle::fromDirectory()}
	...
	public static function contributeKnowledge( array $bundles ): array {
		...
		$bundle = KnowledgeBundle::fromDirectory( dirname( __DIR__ ) . '/knowledge', 'contact-form-7' );
