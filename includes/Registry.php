...
use GalatanOvidiu\AbilitiesCatalogCf7\Contracts\Ability;
...
final class Registry {
	 ...
	 *
	 * @var array<string,\GalatanOvidiu\AbilitiesCatalogCf7\Contracts\Ability>
	 */
	 ...
	 *
	 * @var array<int,\GalatanOvidiu\AbilitiesCatalogCf7\Contracts\CategoryProvider>
	 */
			...
			if ( ! $is_readonly && ! $has_annotated_safety ) {
				_doing_it_wrong(
					...
					sprintf(
						...
						esc_html__( 'Ability "%s" was refused: it must be read-only or a write that explicitly sets annotations.destructive to a boolean. A write that omits the destructive annotation is treated as unsafe and not registered.', 'abilities-catalog-cf7' ),
						esc_html( $name )
					),
					'0.1.0'
				);
	 ...
	 * load.
	 *
	 * @param \GalatanOvidiu\AbilitiesCatalogCf7\Contracts\Ability $ability The discovered ability instance.
	...
	private function discover(): void {
		$base = ABILITIES_CATALOG_CF7_DIR . 'includes/Abilities/';
